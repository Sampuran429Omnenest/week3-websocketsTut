...
const SECTORS = ["IT","Bank","Auto","Energy","Pharma","FMCG","Metal","Realty"];

for (let i = 1; i <= 480; i++) {
  const sym = `STK${String(i).padStart(3, "0")}`;
  const basePrice = parseFloat((100 + Math.random() * 4900).toFixed(2));
  stocks[sym] = {
    symbol:    sym,
    name:      `Load Test Corp ${i}`,
    sector:    SECTORS[i % SECTORS.length],
    price:     basePrice,
    marketCap: Math.floor(Math.random() * 1000000),
    // Fields below will be filled by the initialisation loop
    open: 0, high: 0, low: 0, prevClose: 0, volume: 0, change: 0, changePercent: 0
  };
}
...
server.on("connection", (client) => {
  ...
  Object.values(stocks).forEach((s) => {
    client.send(JSON.stringify({ type: "STOCK_UPDATE", stock: s, serverTs:Date.now() }));
  });
...
setInterval(() => {
  ...
  toUpdate.forEach((sym) => {
    ...
    broadcast({ type: "STOCK_UPDATE", stock: stocks[sym] , serverTs:Date.now() });
  });
}, 400);
